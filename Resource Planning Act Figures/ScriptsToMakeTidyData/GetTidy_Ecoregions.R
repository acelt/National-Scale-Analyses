...
#Organize the data
path <-  "C:\\Users\\alaurencetraynor\\Documents\\LMF\\"
estimates <- paste0(path, "2021 Estimates by Ecoregion.csv")
...
est$MEDITER.RANEAN.CALIFORNIA <- as.character(est$MEDITER.RANEAN.CALIFORNIA)
...
#Change anything reading as int to chr
grouse$MARINE.WEST.COAST.FOREST <- as.character(grouse$MARINE.WEST.COAST.FOREST)
#Change anything reading as int to chr
grouse$MEDITER.RANEAN.CALIFORNIA <- as.character(grouse$MEDITER.RANEAN.CALIFORNIA)
...
est_tall_nresp["Year"] <- Year

#We probably dont need this but just in case

write.csv(est_tall_nresp, file = paste0(path, "NumberResponses_Ecoregions_CatVariables_", Year,".csv"))

# Read in the other data set, merge with  data

FullYears <- read.csv(file = "C:\\Users\\alaurencetraynor\\Documents\\LMF\\RPA\\EstimatesByEcoregion_2012_2020_withlabels.csv")
